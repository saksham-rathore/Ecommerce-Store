...
const Navbar = () => {
  return (
    <header className="fixed top-0 left-0 right-0 z-50 flex justify-center px-4 pt-4">
      <nav className="w-full max-w-5xl flex items-center justify-between gap-8 px-6 py-3 rounded-2xl bg-[#0d0d14]/80 backdrop-blur-xl border border-violet-500/20 shadow-[0_8px_32px_rgba(109,40,217,0.25)]">
        ...
        <ul className="flex items-center gap-1">
          {["New In", "Collections", "Women", "Men", "About"].map((item) => (
            <li key={item}>
              <a
                href="#"
                className="relative px-4 py-2 text-sm font-medium text-white/50 hover:text-white transition-colors duration-200 rounded-lg hover:bg-violet-500/10 group block"
              >
                {item}
                {/* animated underline */}
        ...
        <div className="flex items-center gap-3 shrink-0">
          {/* Cart button */}
          <button className="relative p-2 rounded-xl bg-violet-500/10 border border-violet-500/20 text-white/60 hover:text-white hover:bg-violet-500/20 hover:border-violet-400/40 transition-all duration-200 hover:scale-105 active:scale-95">
